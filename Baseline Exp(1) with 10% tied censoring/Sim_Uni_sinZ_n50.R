...
for(b in 1:B){
  ...
  IC = rbinom(n=n,size=1,prob=1-0.1); 
  C = IC*C + (1-IC)*sample(1:19,size=n,replace=TRUE)/20*upper_unif_censoring; 
  gZ = gfunction(Z); 
  Exp_i = exp(gZ); 
...
write.csv(PLRT.power, "Sim_Uni_sinZ_n50.csv")