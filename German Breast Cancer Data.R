...
gbsg = na.omit(gbsg)
hfit = coxph(Surv(rfstime, status) ~ size, data= gbsg)
hfit$coefficients 
ifit = isoph(Surv(rfstime, status) ~ iso(size,shape="inc"), data= gbsg)


plot((ifit$iso.cov)$size, (ifit$iso.cov)$psi.hat,type="s", 
     xlab="size",ylab="risk effect in size")
abline(a=-hfit$coefficients*ifit$Zk ,b=hfit$coefficients )


Data = cbind(gbsg$rfstime,gbsg$status,gbsg$size)
...
#Test_bb = mclapply(1:BB, do_bb, mc.cores = 2)
Test_bb = sapply(1:BB, do_bb)
Tn_b = unlist(Test_bb); quantile(Tn_b,probs=0.95) #9.507811

hist(Tn_b); abline(v=Tn); #9.884154
...
as.numeric(mt_resids$pval.test<0.05)
